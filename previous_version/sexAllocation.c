...
#define VERSION "25.04.2016"
#define DEPENDENCY "Genepop version 4.5.1\n\t\t\t\tdiveRsity.R"
...
void statisticsPopulations(Deme* population, const int nDemes, const int maxIndPerDem, const int nQuantiLoci, const int fecundity, const double migration, const double extinction, const int recolonization, const int sexualSystem, const double sexAvantage, const int seed, int time){
	...
	if(fichierSortie == NULL){
		...
		fprintf(fichierSortie, "nDemes\tnInd\tnQuantiLoci\tfecundity\tmigRate\textRate\trecolonization\tatGeneration\tsexSystem\tsexAvantage\tseed\tmeanFemAlloc\tsdFemAlloc\tmeanFemAllocCosexual\tsdFemAllocCosexual\tcosexualProportion\texpFST\n");
	...
	if(fichierSortie != NULL){	
		...
		fprintf(fichierSortie, "%d\t%d\t%d\t%d\t%lf\t%lf\t%d\t%d\t%d\t%lf\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", nDemes, maxIndPerDem, nQuantiLoci, fecundity, migration, extinction, recolonization, time, sexualSystem, sexAvantage, seed, meanAllocFemale, sdAllocFemale, meanAllocFemaleCosexual, sdAllocFemaleCosexual, cosexualProportion, fstValue);
		fclose(fichierSortie);
	}
}
